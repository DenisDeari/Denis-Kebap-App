...
import { getLocationById, updateLocation } from "@/lib/storage";
...
export default function EditLocationPage() {
  ...
  useEffect(() => {
    ...
    if (currentLocation) {
      ...
      const defaultOpenDays = daysOfWeek.map((day) => ({
        day,
        isOpen: day !== "Sunday",
        openTime: "11:00",
        rushStart: "11:15",
      ...
      setFormData({
        ...
        tagLine: currentLocation.tagLine,
        orderReadyIn: currentLocation.orderReadyIn,
        orderCancelBefore: currentLocation.orderCancelBefore,
        bufferTime: currentLocation.bufferTime,
        rushHourDisplaySeconds: currentLocation.rushHourDisplaySeconds || 0,
        regularDisplaySeconds: currentLocation.regularDisplaySeconds || 0,
        openDays: currentLocation.openDays || defaultOpenDays,
      });
    }
  }, [locationId]);
  ...
  if (!location) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <p className="text-gray-600">Standort nicht gefunden</p>
      </div>
    );
  }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Generiere address automatisch aus openDays, wenn openDays vorhanden sind
    const generatedAddress = formData.openDays && formData.openDays.length > 0 && location
      ? formatOpeningHours({ ...location, openDays: formData.openDays } as Location)
      : formData.address;
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-6">
        ...
        <h1 className="text-3xl font-bold text-gray-900">Edit Location Details</h1>
      </div>
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <div className="w-full px-4 py-2 border border-gray-300 rounded-lg bg-gray-50 text-gray-700 font-mono text-sm">
              {formData.openDays && formData.openDays.length > 0 && location ? (
                formatOpeningHours({ ...location, openDays: formData.openDays } as Location)
              ) : (
                formData.address || "Keine Öffnungszeiten definiert"
              )}
            </div>
            <p className="mt-1 text-xs text-gray-500">
              Die Öffnungszeiten werden automatisch aus den &quot;Select Open Days&quot; Einstellungen unten generiert.
              Diese Vorschau zeigt, wie die Zeiten in der Anwendung angezeigt werden.
            </p>
          </div>
