...
import { getIngredients, updateIngredient, deleteIngredient } from "@/lib/storage";
import { Ingredient } from "@/types";
import { formatPrice } from "@/lib/utils";

// Sortable Row Component
function SortableRow({ ingredient, onToggleStatus, onDelete }: {
  ingredient: any;
  ...
  return (
    <tr ref={setNodeRef} style={style} className="hover:bg-gray-50">
      ...
      <td className="px-6 py-4 whitespace-nowrap">
        <div className="text-sm text-gray-900">{formatPrice(ingredient.price)}</div>
      </td>
      <td className="px-6 py-4 whitespace-nowrap">
        <div className="text-sm text-gray-600">{ingredient.createdOn}</div>
      </td>
      <td className="px-6 py-4 whitespace-nowrap">
        <button
          onClick={() => onToggleStatus(ingredient.id)}
          className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
            ingredient.status ? "bg-gray-900" : "bg-gray-300"
          }`}
        >
          <span
            className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
              ingredient.status ? "translate-x-6" : "translate-x-1"
            }`}
          />
        </button>
      </td>
...
export default function IngredientsListPage() {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      ...
      // Reordering is temporarily disabled in Full-Stack version 
      // until explicit ordering column is added
      // reorderIngredients(newOrder.map((i) => i.id));
    }
  };
  ...
  const handleToggleStatus = async (id: string) => {
    ...
    if (ingredient) {
      await updateIngredient(id, { status: !ingredient.status });
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Zeige {paginatedIngredients.length} von {filteredIngredients.length} Ingredients
          </p>
        </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        {filteredIngredients.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-gray-500">Keine Ingredients gefunden.</p>
          </div>
        ) : (
          <>
            <DndContext
